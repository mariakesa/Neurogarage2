...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    """
    # Your code here
    linear_combination = np.dot(theta, feature_vector) + theta_0
    z= label*linear_combination
    return max(0, 1-z)
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    linear_combination =theta@feature_matrix.T + theta_0
    z= labels*linear_combination
    return np.maximum(np.zeros(z.shape), np.ones(z.shape)-z).mean()
...
def average_perceptron(feature_matrix, labels, T):
    ...
    for t in range(T):
        for i in get_order(nsamples):
            theta, theta_0 = perceptron_single_step_update(feature_matrix[i], labels[i], theta, theta_0)
            theta_container.append(theta)
            theta_0_container.append(theta_0)
    return np.mean(theta_container, axis=0), np.mean(theta_0_container)
...
def classify(feature_matrix, theta, theta_0):
    ...
    """
    # Your code here
    factor=feature_matrix@theta + theta_0
    classification = np.where(factor > 1e-7, 1, -1)
