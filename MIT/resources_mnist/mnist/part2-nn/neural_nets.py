...
class NeuralNetwork():
    ...
    def train(self, x1, x2, y):
        ...
        # Compute gradients
        output_layer_error = np.multiply((activated_output - y), np.vectorize(output_layer_activation_derivative)(output)) # TODO
        hidden_layer_error = np.multiply((np.transpose(self.hidden_to_output_weights) * output_layer_error), np.vectorize(rectified_linear_unit_derivative)(hidden_layer_weighted_input)) # TODO

        bias_gradients = hidden_layer_error # TODO
        hidden_to_output_weight_gradients = output_layer_error * np.transpose(hidden_layer_activation) # TODO
